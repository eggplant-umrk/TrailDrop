...
-- Idempotency key for POST /reservations (one per reservation operation,
-- generated by the Frontend). Nullable; NULLs are distinct in the unique
-- index. See supabase/migrations/20260928090000_reservation_idempotency_key.sql.
alter table public.reservations
...
-- reserved forever unless someone explicitly cancels them. pg_cron runs
-- this every 10 minutes (scheduled at the end of this file); the
-- staff-authenticated Backend endpoint (POST /staff/reservations/
-- expire-stale) can also call it on demand (see the README runbook).
